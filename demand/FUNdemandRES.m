function [PDresyR1, PDresyM1, EDdayWR1, EDdaySR1, EDyearR1] = FUNdemandRES(r)
...
FDresyR1 = zeros(1,35040);
    ...
    for i = 1:365
        ...
        FDresR14= zeros(1,96);
            ...
            for k = 1:96-dd
                FDresR14(1,dd+k) = FDresR13(k);
            end

            for k = 1:dd
                FDresR14(1,k) = FDresR13(96-dd+k);
            end
...
%% Energy use (In MW)
PDresR11 = ((1+G(r))*R(r))*(1-E(r))*(P(r))*FDresR11*0.001;
PDresR12 = ((1+G(r))*R(r))*(1-E(r))*(P(r))*FDresR12*0.001;
PDresyR1 = ((1+G(r))*R(r))*(1-E(r))*(P(r))*FDresyR1*0.001;
...
%% costs?
% I see no reason for extra costs (just nominal improvements of technology
%CPR = R(r)*G(r)*

%% Plot Power curve %percentage
%{
%window
figure('Name','Residential Demand')
...
title('Day curve')
xlabel('Time of Day')
ylabel('% of peak power')

%plot change in season
subplot(2,3,2)
plot(aa,'y')
hold on
plot(bb,'g')
plot(cc)
xticks([1 365/4 365/2 (3*365)/4 365])
xticklabels({'00:00','6:00','12:00','18:00','24:00'})
title('curve change')
xlabel('Time of Day')
ylabel('% of peak power')

%plot year with curve change
subplot(2,3,4)
plot(rdv)
